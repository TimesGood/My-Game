...
public class LightHandler : Singleton<LightHandler>
{
    ...
    public ChunkManager chunkManager => ChunkManager.Instance;
    ...
    {
        StartCoroutine(UpdateScopeLight(0, chunkManager.Width - 1, 0, chunkManager.Height - 1));
    }

    IEnumerator LightUpdate(Vector2Int pos) {
        //更新该光源一定范围的光亮
        int px1 = Mathf.Clamp(pos.x - (int)sunlight, 0, chunkManager.Width - 1);//最左
        int px2 = Mathf.Clamp(pos.x + (int)sunlight, 0, chunkManager.Width - 1);//最右
        int py1 = Mathf.Clamp(pos.y - (int)sunlight, 0, chunkManager.Height - 1);//最下
        int py2 = Mathf.Clamp(pos.y + (int)sunlight, 0, chunkManager.Height - 1);//最上
    ...
    private IEnumerator UpdateScopeLight(int minX, int maxX, int minY, int maxY)
    {
        ...
        for (int x = minX; x <= maxX; x++)
        {
            for (int y = minY; y <= maxY; y++)
            {
                ...
                {
                    //获取四周光亮值
                    int nx1 = Mathf.Clamp(x - 1, 0, chunkManager.Width - 1);
                    int nx2 = Mathf.Clamp(x + 1, 0, chunkManager.Width - 1);
                    int ny1 = Mathf.Clamp(y - 1, 0, chunkManager.Height - 1);
                    int ny2 = Mathf.Clamp(y + 1, 0, chunkManager.Height - 1);
                    //取最亮的一边
