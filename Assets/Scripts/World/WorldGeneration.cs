...
public class WorldGeneration : Singleton<WorldGeneration>, ISaveManager
{
    ...
    public void Erase(Layers layer, int x, int y) {
        ...
        tilemaps[(int)layer].SetTile(pos, null);
        //自发光瓦片处理
        ...
        if (layer == Layers.Ground) {
            foreach (var dir in directions) {
                ...
                LiquidClass tileClass = GetTileClass(Layers.Liquid, target.x, target.y) as LiquidClass;
                if (tileClass != null) {
                    if(LiquidHandler.Instance.CheckMarkForUpdate(tileClass, (Vector2Int)target)) continue;
                    List<Vector3Int> tiles = FindConnectedTiles(Layers.Liquid, target);
                    //激活该区液体
                    foreach (var item in tiles) {
                        LiquidHandler.Instance.MarkForUpdate(tileClass, (Vector2Int)item);
                    }
                }
        }
        }
    ...
    public List<Vector3Int> FindConnectedTiles(Layers layer, Vector3Int startPosition) {
        // 初始化数据结构
        var connectedTiles = new List<Vector3Int>();//结果
        var visited = new HashSet<Vector3Int>();//记录走过的路径
